...
(def VM {
					...
					"unmap" (fn [m & r] {:pre [(map? m)]}
										(conj r (seq (reduce concat (seq m)))))
					...
					;; Words for Words
					"word" (fn [s & r] {:pre [(consize.core/wordstack? s)]} (conj r (reduce str s))),
					...
					"print" (fn [w & r] {:pre [(string? w)]} (do (print w) (seq r))),
					"flush" (fn [& r] (do (flush) (seq r))),
					...
					"spit" (fn [file data & r] {:pre [(string? file) (string? data)]}
									 (do (spit file data) (seq r))),
					"spit-on" (fn [file data & r] {:pre [(string? file) (string? data)]}
											(do (spit file data :append true) (seq r))),
									 ...
									 (conj r
												 (let [runcc
													 ...
													 (fn [& ds]
														 ...
														 (runcc qt (seq (if (nil? ds) "" ds)) dict))))),

					;; Arithmetics
					"integer?" (fn [w & r]
											 (conj r (if (string? w) (if (integer? (read-string w)) "t" "f") "f"))),
					"+" (consize.core/binary +), "-" (consize.core/binary -), "*" (consize.core/binary *),
					"div" (consize.core/binary quot), "mod" (consize.core/binary mod),
					"<" (consize.core/pred <), ">" (consize.core/pred >), "==" (consize.core/pred ==), "<=" (consize.core/pred <=), ">=" (consize.core/pred >=),
